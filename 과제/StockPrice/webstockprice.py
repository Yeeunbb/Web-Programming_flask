...
@app.route("/getprice/")
@app.route("/getprice/<query>/<dayss>")
def getprice(query, dayss):
    print ("<query={}><days={}>".format(query, dayss))
    selList = query.split('&')        
    if not set(selList).issubset(utils.stockNames):
        #print (selList)
        return {}
    url = GETPRICE_URL.format(dayss)
    print ("<url={}><days={}>".format(url, dayss))
    priceJson = urlopen(url).read().decode("utf-8")
    #print (priceJson)
    
    priceDF = pd.read_json(priceJson, orient='split')
    headList = ['date'] + list(priceDF[selList].columns)
    rowList = []
    for da in priceDF[selList].index:
        pr_list = priceDF[selList].loc[da]
...
@app.route("/getplot/<query>/<dayss>")
def getplot(query, dayss):
